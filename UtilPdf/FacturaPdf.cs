...
void ComposeHeader(IContainer container)
{
    container.PaddingBottom(10).Row(row =>
    {
        ...
        row.ConstantItem(150)
           .Column(column =>
           {
               column.Item().AlignRight().Text("FACTURA ELECTRÓNICA").Bold().FontSize(14);
               column.Item().AlignRight().Text($"F001-{_venta.Id:D8}").Bold(); // Número de la boleta
           });
    });
}
...
void ComposeContent(IContainer container)
{
    container.PaddingVertical(10).Column(column =>
    {
        ...
        column.Item().Row(row =>
        {
            ...
            {
                column.Item().Text($"Cliente: {_venta.IdClienteNavigation.IdTablaPersonaJuridicaNavigation.RazonSocial} -{_venta.IdClienteNavigation.IdTablaPersonaJuridicaNavigation.Ruc}");
...
void ComposeFooter(IContainer container)
{
    container.PaddingTop(10).Column(column =>
    {
        ...
        column.Item().Text("REPRESENTACIÓN IMPRESA DE LA FACTURA ELECTRÓNICA").FontSize(8).Italic();
